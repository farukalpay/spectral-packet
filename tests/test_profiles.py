...
def test_modal_tail_supports_complex_coefficients() -> None:
    coefficients = torch.tensor([1.0 + 1.0j, 1.0 - 2.0j], dtype=torch.complex128)
    tail = modal_tail(coefficients)

    torch.testing.assert_close(tail, torch.tensor([5.0 / 7.0, 0.0], dtype=torch.float64))
...
def test_profile_statistics_reject_non_positive_mass() -> None:
    grid = torch.linspace(0.0, 1.0, 5, dtype=torch.float64)
    zero_profile = torch.zeros_like(grid)

    with pytest.raises(ValueError, match="positive mass"):
        profile_mean(zero_profile, grid)

    with pytest.raises(ValueError, match="positive mass"):
        profile_variance(zero_profile, grid)


def test_relative_l2_error_rejects_zero_norm_reference() -> None:
    grid = torch.linspace(0.0, 1.0, 5, dtype=torch.float64)
    zero_profile = torch.zeros_like(grid)

    with pytest.raises(ValueError, match="positive L2 norm"):
        relative_l2_error(zero_profile, zero_profile, grid)


def test_profile_validation_rejects_non_finite_values() -> None:
    grid = torch.linspace(0.0, 1.0, 5, dtype=torch.float64)
    bad_profile = torch.tensor([0.0, 1.0, float("nan"), 0.0, 0.0], dtype=torch.float64)

    with pytest.raises(ValueError, match="profiles must be finite"):
        profile_mass(bad_profile, grid)
