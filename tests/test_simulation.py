...
def test_single_time_propagate_rejects_time_vectors() -> None:
    domain, _, _, propagator, _ = _build_reference_stack(num_modes=16, quadrature_points=1024)
    state = SpectralState(domain, torch.ones(16, dtype=domain.complex_dtype))

    with pytest.raises(ValueError, match="single time value"):
        propagator.propagate(state, torch.tensor([0.0, 0.1], dtype=domain.real_dtype))
