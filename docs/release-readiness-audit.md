...
- added first-class spectral feature export and tree-model train/tune workflows across Python, CLI, MCP, and API,
- made feature-export artifacts write an explicit `features_schema.json` contract with ordered columns, dtypes, and semantic meaning,
- made feature-export and tree-model artifacts record input kind, normalization, mode count, library versions, and any observed ordering policy,
...
- spectral feature export plus tree-model train/tune workflows are beta surfaces rather than part of the stable core,
- the scikit-learn baseline is the most directly validated tree-model backend in this release,
