...
| MCP server | Supported. Local stdio and supervised streamable-HTTP deployments are first-class. | Best-effort local stdio use; supervised long-running deployment is not a first-class promise | Supported. Local stdio and launchd-supervised streamable-HTTP deployments are first-class. |
...
- preferred target for supervised streamable-HTTP MCP deployment with `systemd` or a container restart policy.
...
- `launchd` is the preferred external supervisor for long-running streamable-HTTP MCP deployments.
