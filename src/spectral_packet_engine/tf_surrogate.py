    ...
    *,
    ...
    if enable_memory_growth:
        for device in gpu_devices:
            try:
                ...
                _log.warning("Failed to enable memory growth on GPU device %s", device.name)
    ...
    if enable_xla:
        try:
            ...
            _log.warning("Failed to enable XLA JIT compilation")
            xla_enabled = False
    ...
    if enable_mixed_precision and gpu_devices:
        try:
            ...
            _log.warning("Failed to set mixed_float16 precision policy, falling back to float32")
