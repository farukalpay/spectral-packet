...
def _tree_model_check(device: str) -> ReleaseGateCheckResult:
    """Validate the spectral-feature-export → tree-train pipeline."""
    from spectral_packet_engine.artifacts import write_feature_table_artifacts

    with TemporaryDirectory(prefix="release_gate_tree_") as tmp:
        output_dir = Path(tmp)
        grid = np.linspace(0.0, 1.0, 20, dtype=np.float64)
        times = np.linspace(0.0, 0.5, 6, dtype=np.float64)
        profiles = []
        for c, w in zip(np.linspace(0.2, 0.6, 6), np.linspace(0.05, 0.09, 6)):
            p = np.exp(-((grid - c) ** 2) / (2 * w**2))
        ...
        table = ProfileTable(
            ...
            sample_times=times,
            profiles=np.asarray(profiles, dtype=np.float64),
            source="release-gate-tree",
        )

        feature_summary = export_feature_table_from_profile_table(
            table, num_modes=8, device=device,
        )
        feature_dir = output_dir / "features"
        write_feature_table_artifacts(feature_dir, feature_summary)
        features_csv = feature_dir / "features.csv"
        schema_json = feature_dir / "features_schema.json"
        assert features_csv.exists(), "features.csv not written"
        assert schema_json.exists(), "features_schema.json not written"

        # Add synthetic target column for tree training
        import csv
        rows = []
        with open(features_csv) as f:
            ...
            for row in reader:
                row["target"] = str(float(row.get("time", 0)) * 2 + 1)
                rows.append(row)
        fieldnames.append("target")
        features_with_target = output_dir / "features_with_target.csv"
        with open(features_with_target, "w", newline="") as f:
            writer = csv.DictWriter(f, fieldnames=fieldnames)
            writer.writeheader()
            writer.writerows(rows)

        tree_summary = train_tree_model(
            features_with_target,
            target_column="target",
            library="sklearn",
            task="regression",
            export_dir=output_dir / "tree_train",
        )
        assert tree_summary is not None, "tree training returned no summary"

    return ReleaseGateCheckResult(
        name="tree_model_pipeline",
        surface="tree-model",
        required=True,
        status="passed",
        detail="Exported spectral features, validated schema, trained sklearn tree model.",
        metrics={
            "num_features": feature_summary.num_features,
            "num_rows": feature_summary.num_rows,
        },
    )
