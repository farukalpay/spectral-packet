...
def normalize_profiles(profiles, grid) -> Tensor:
    spatial_grid = _validate_grid(grid)
    values = _validate_profiles(profiles, spatial_grid)
    mass = profile_mass(values, spatial_grid)
    if torch.any(mass <= 0).item():
        raise ValueError("profiles with non-positive mass cannot be normalized")
...
def summarize_profile_compression(
    ...
    *,
    ...
    for count in counts.tolist():
        basis = InfiniteWellBasis(domain, int(count))
        coeffs = project_profiles_onto_basis(values, spatial_grid, basis)
        reconstruction = reconstruct_profiles_from_basis(coeffs, spatial_grid, basis)
        error = relative_l2_error(values, reconstruction, spatial_grid)
