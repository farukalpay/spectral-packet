...
def sanitize_filename(name: str) -> str:
    """Ensure filename is safe — no path components, no special chars."""
    name = name.strip()
    if not name:
        raise ValueError("Filename must not be empty")
    if "/" in name or "\\" in name or ".." in name:
        raise ValueError(f"Filename contains path traversal characters: {name!r}")
    if not _SAFE_FILENAME_RE.match(name):
        raise ValueError(
            f"Filename contains unsafe characters: {name!r}. "
            f"Use only alphanumeric, underscore, hyphen, and dot."
        )
    return name
